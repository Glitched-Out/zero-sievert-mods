...
if (room != r_menu)
{
    if instance_exists(obj_player)
    {
        if (obj_player.state == gml_Script_scr_player_state_inventory)
        {
                ...
                if instance_exists(obj_item)
                {
                    ...
                    for (i = 0; i < ds_list_size(_list_id); i++)
                    {
                        ...
                        if (_price_kg >= 0)
                            _col = c_red
                        if (_price_kg >= 400)
                            _col = merge_colour(c_red, c_lime, 0.25)
                        if (_price_kg >= 1000)
                            _col = merge_colour(c_red, c_lime, 0.5)
                        if (_price_kg >= 3200)
                            _col = merge_colour(c_red, c_lime, 0.75)
                        if (_price_kg >= 8000)
                            _col = c_lime
                        if (_price_kg >= 13000)
                            _col = c_aqua
            ...
            if scr_mouse_inside((camx + global.show_overlay_price_slot_x), (camy + global.show_overlay_price_slot_y), global.show_overlay_price_slot_w, global.show_overlay_price_slot_h)
            {
                if instance_exists(obj_item)
                {
                    ...
                    for (i = 0; i < ds_list_size(_list_id); i++)
                    {
                        ...
                        var _price_slot = ((_price / 5) / _slots)
                        _col = c_white
                        if (_price_slot >= 0)
                            _col = c_red
                        if (_price_slot >= 40)
            ...
            if scr_mouse_inside((camx + global.show_overlay_max_price_slot_x), (camy + global.show_overlay_max_price_slot_y), global.show_overlay_max_price_slot_w, global.show_overlay_max_price_slot_h)
            {
                if instance_exists(obj_item)
                {
                    _list_id = ds_list_create()
                    ...
                    for (i = 0; i < ds_list_size(_list_id); i++)
                    {
                        ...
                        _col = c_white
                        if (_max_price_slot >= 0)
                            _col = c_red
                        if (_max_price_slot >= 50)
                            _col = merge_colour(c_red, c_lime, 0.25)
                        if (_max_price_slot >= 220)
                            _col = merge_colour(c_red, c_lime, 0.5)
                        if (_max_price_slot >= 600)
                            _col = merge_colour(c_red, c_lime, 0.75)
                        if (_max_price_slot >= 1000)
                            _col = c_lime
                        if (_max_price_slot >= 1520)
                            _col = c_aqua
                        if (_max_price_slot >= 1900)
                            _col = c_fuchsia
                        _xx = ds_list_find_value(_list_xx, i)
                        _yy = ds_list_find_value(_list_yy, i)
                        _x = ds_list_find_value(_list_x, i)
